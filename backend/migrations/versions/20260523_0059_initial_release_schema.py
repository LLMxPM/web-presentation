...
Revises: 
Create Date: 2026-05-23 15:43:58.921923
...
def upgrade() -> None:
    ...
    op.create_table('ai_agent_run_tasks',
    ...
    op.create_index(op.f('ix_ai_llm_configs_provider_key'), 'ai_llm_configs', ['provider_key'], unique=False)
    ...
    sa.Column('component_type', sa.String(length=64), server_default='内容区块', nullable=False),
    ...
    # ### Alembic 自动生成的基线建表命令结束。 ###


def downgrade() -> None:
    """移除首次发布基线创建的全部业务表、约束与索引。"""
    # ### Alembic 自动生成的基线回滚命令。 ###
    ...
    op.drop_index(op.f('ix_ai_agent_run_events_task_id'), table_name='ai_agent_run_events')
    ...
    op.drop_index(op.f('ix_ai_llm_configs_provider_key'), table_name='ai_llm_configs')
    ...
    op.drop_index(op.f('ix_ai_agent_run_tasks_workspace_id'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_user_id'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_status'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_source'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_session_id'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_scope_type'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_run_id'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_project_id'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_page_id'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_component_id'), table_name='ai_agent_run_tasks')
    op.drop_index(op.f('ix_ai_agent_run_tasks_agent_id'), table_name='ai_agent_run_tasks')
    op.drop_table('ai_agent_run_tasks')
    op.drop_table('workspaces')
    op.drop_table('users')
    # ### Alembic 自动生成的基线回滚命令结束。 ###
