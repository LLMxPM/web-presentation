...
async def test_apply_page_edits_tool_should_accept_explicit_page_id(authenticated_client: AsyncClient) -> None:
...
async def test_apply_page_edits_should_reject_stale_base_version(authenticated_client: AsyncClient) -> None:
...
async def test_ai_session_messages_should_preserve_tool_metadata(authenticated_client: AsyncClient, monkeypatch) -> None:
    ...
    runtime_messages = runtime_response.json()["messages"]
    assert [message["role"] for message in runtime_messages] == ["assistant", "tool"]
    assert runtime_messages[0]["tool_calls"] == assistant_message["tool_calls"]
    assert runtime_messages[1]["tool_call_id"] == "tool-assets-1"
    assert runtime_messages[1]["tool_calls"] == []
...
async def test_ai_agent_run_events_should_be_normalized_for_editor() -> None:
    ...
    tool_event = facade._normalize_event(
        raw_event={
            ...
            "run_id": "agent-run-1",
            "session_id": "agent-session-1",
            "content": "preview_component_edits(component_id=6, edits=..., change_note=...) completed in 0.0111s.",
            "tool": {
...
async def test_ai_cancelled_runtime_snapshot_should_restore_tool_details(authenticated_client: AsyncClient) -> None:
    """停止前的工具事件应从 runtime snapshot 恢复，且不伪造成 Agno tool 消息。"""

    workspace_id = await _create_workspace(authenticated_client, "AI 停止工具恢复工作空间")
    project_id = await _create_project(authenticated_client, workspace_id, "AI 停止工具恢复项目")
    page_id = await _create_page(
        authenticated_client,
        workspace_id=workspace_id,
        project_id=project_id,
    ...
    async with get_session_factory()() as db_session:
        ...
        task = await service.create_task(
            ...
            scope=scope,
            input_summary="列出素材",
            input_payload_json=build_agent_run_input_payload(message="列出素材", image_attachment_ids=[]),
        )
        await service.append_event(run_id=run_id, event=AgentRunEvent(event="run.started", run_id=run_id, session_id=session_id))
        await service.append_event(
            run_id=run_id,
            event=AgentRunEvent(
                ...
                run_id=run_id,
                session_id=session_id,
                data={"tool_name": "list_workspace_render_assets", "tool_args": {"workspace_id": workspace_id}},
            ),
        )
        await service.append_event(
            run_id=run_id,
            event=AgentRunEvent(
                ...
                run_id=run_id,
                session_id=session_id,
                data={"tool_name": "list_workspace_render_assets", "result": {"total": 2}},
            ),
        )
        await service.mark_cancelling(task=task)
    ...
    assert [item["role"] for item in runtime_payload["messages"]] == ["user", "assistant"]
    assert runtime_payload["messages"][1]["content"] == ""
    tool_details = runtime_payload["tool_details"]
    assert len(tool_details) == 1
    assert tool_details[0]["run_id"] == run_id
    assert tool_details[0]["tool_name"] == "list_workspace_render_assets"
    assert tool_details[0]["status"] == "completed"
    assert tool_details[0]["input_payload"] == {"workspace_id": workspace_id}
    assert tool_details[0]["output_payload"] == {"total": 2}
    assert tool_details[0]["assistant_message_id"] == runtime_payload["messages"][1]["id"]


async def test_ai_tool_details_should_keep_separate_missing_call_id_pairs(authenticated_client: AsyncClient) -> None:
    ...
    async with get_session_factory()() as db_session:
        ...
        await service.create_task(
            ...
            backend_session_id=None,
            scope=scope,
            input_summary="连续读取资源",
            input_payload_json=build_agent_run_input_payload(message="连续读取资源", image_attachment_ids=[]),
        )
        await service.append_event(run_id=run_id, event=AgentRunEvent(event="run.started", run_id=run_id, session_id=session_id))
        await service.append_event(
            run_id=run_id,
            event=AgentRunEvent(
                ...
                run_id=run_id,
                session_id=session_id,
                data={"tool_name": "list_workspace_render_assets", "tool_args": {"page": 1}},
            ),
        )
        await service.append_event(
            run_id=run_id,
        ...
        await service.append_event(
            ...
            event=AgentRunEvent(
                ...
                run_id=run_id,
                session_id=session_id,
                data={"tool_name": "list_workspace_render_assets", "tool_args": {"page": 2}},
            ),
        )
        await service.append_event(
            run_id=run_id,
        ...
        details = await service.list_tool_details_for_session(
            session_id=session_id,
            agent_id=AGENT_COORDINATOR_AGENT_ID,
            user_id=1,
        )
    ...
    assert [detail.input_payload for detail in details] == [{"page": 1}, {"page": 2}]
    assert [detail.output_payload for detail in details] == [{"total": 1}, {"total": 2}]
    assert all(detail.tool_call_id is None for detail in details)
