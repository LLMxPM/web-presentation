...
from app.ai.auth_tokens import COMPONENT_TOOL_WRITE_SCOPES, build_agent_tool_token
...
async def test_archive_component_tool_should_pass_user_context_and_soft_archive(
    authenticated_client: AsyncClient,
) -> None:
    """归档组件工具应把当前用户传给服务层，并让组件从普通读取接口中消失。"""
    ...
    archive_tool = _find_tool(build_component_manager_tools(get_session_factory()), "archive_component")
    ...
    dependencies["agent_id"] = AGENT_COORDINATOR_AGENT_ID
    dependencies["tool_auth_token"] = build_agent_tool_token(
        ...
        scopes=COMPONENT_TOOL_WRITE_SCOPES,
    )

    result = await archive_tool.entrypoint(run_context, component_id=component["id"])
