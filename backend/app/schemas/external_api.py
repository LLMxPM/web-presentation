...
class ExternalMutationJobResponse(BaseModel):
    ...
    status: Literal["pending", "running", "finalizing", "succeeded", "failed", "canceled"] = Field(
        ..., description="任务当前状态"
    )
    attempt_count: int = Field(..., description="已重试次数")
...
class ExternalGuideResponse(BaseModel):
    ...
    operations: list[dict[str, Any]] = Field(default_factory=list, description="所有可用操作列表及元数据规格")
