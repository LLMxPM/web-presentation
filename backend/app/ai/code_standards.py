...
- 组件复用判断与页面布局判断是两个独立步骤：先判断是否应复用或创建重复内容页模板，再在页面组件或页面源码内部划分标题区、主体区和辅助区。固定画布中的标题、眉题和导航可以顶部对齐，但主体内容区、内容面板和稀疏卡片默认应在各自的可用高度内上下平衡。不要把整个页面机械地 `justify-center`；应在正确的局部区域内平衡完整内容组。
- 对 `flex-col`，垂直方向由 `justify-*` 控制、水平方向由 `items-*` 控制；对 Grid，垂直居中使用 `place-items-center` 或 `items-center`；`text-center` 只改变文字对齐，不会使容器内容上下居中。卡片表面可以为了等高而拉伸，但卡片内部内容组仍需有明确的垂直对齐策略。
- `flex-1` 只用于分配已经规划好的页面区域，不得作为制造空白或拉伸稀疏卡片的默认手段。禁止把 `grid/flex-1`、固定高度、顶部排列和 `mt-auto` 组合成默认卡片结构，再用自动 margin 把尾部信息推到底部；`mt-auto`、`space-between` 和 `space-around` 只能用于明确的固定页脚、上下锚点或有意的对照构图。
- 内容无法填满容器时，优先将完整内容组垂直居中、缩小容器、调整分区或精简信息；不要让内容贴在顶部并用剩余高度制造空白。只有表格、目录、长文阅读、时间线、密集数据或明确的顶部引导版式，才使用顶部对齐。
...
- `interior_gap` 常由 `mt-auto`、固定高度容器或较大 margin 造成；如果同时出现 `sparse_top_aligned`，表示内容组靠顶部排列而后续空白过大，默认应改为局部垂直居中、取消自动 margin 或重新规划容器高度。由 `space-between`、`space-around` 或 `center` 分布产生的间隙通常是布局意图，但仍需确认它服务于明确构图。`short_last_line`、`single_word_last_line` 已过滤窄容器和居中文本场景，剩余孤行/孤词更值得关注。
...
- 开始创建页面时，先查询并复用已有的页面组件。页面组件不仅用于封面、目录、章节分隔和页面骨架，也用于具有重复标题区、眉题/导航、主体区和辅助区空间关系的内容页模板、报告页模板和数据页模板。
- 如果同一任务包含两个或以上结构相近的内容页，或用户明确要求多页保持统一的内容页版式，即使每页正文、图表和数据不同，也应优先复用已有页面组件；没有合适组件时创建 `component_type=页面组件` 的内容页模板，通过 props 或 slots 接收可变标题和主体内容。重复的是版式结构，不是同一段标题文案；不要因为主体内容不同就把共有的页面壳复制到各页面源码中。
