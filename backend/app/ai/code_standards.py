...
- 固定画布应先划分标题区、主体区和辅助区：标题、眉题和导航可以顶部对齐，但主体内容区、内容面板和稀疏卡片默认应在各自的可用高度内上下平衡。不要把整个页面机械地 `justify-center`；应在正确的局部区域内平衡完整内容组。
...
- 开始创建页面时，先查询并复用已有的页面组件；只有当前页面属于有明确模板价值、预计会在多个页面或项目中复用的封面、目录、章节分隔或页面骨架时，才创建页面组件。
- 编写页面内容时，如果发现同一职责的指标卡、引用卡、图表或表格模块会跨页重复，且变化可以用稳定的 props 或 slots 表达，再沉淀为内容组件；只服务当前页面独特叙事的一次性排版和单纯包装容器直接保留在页面源码中。新组件创建后完成校验、发布并更新项目 suggested_components。
