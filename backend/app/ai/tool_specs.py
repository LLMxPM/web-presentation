...
_COORDINATOR_OPERATION_GUIDES = (
    ...
    _operation_guide(
        "asset",
        "create",
        "创建可通过稳定 name 引用的 SVG 图标、Draw.io、Mermaid、Chart 或 Formula 文本资源。",
        _write_parameters("asset", "create", AssetCreatePayload, action="new"),
        action="new",
        constraints=(
            "asset_type=icon 专指 SVG 图标，不存在 svg 类型；SVG 图片不能伪装成 image 通过该操作创建。",
            "original_name 必须匹配类型：icon=.svg；drawio=.drawio/.xml；mermaid=.mmd/.mermaid/.txt；chart=.json/.yaml/.yml；formula=.tex/.txt。",
            ...
            "不支持直接创建位图 image、video 或 font；保存会话上传图片应查询并使用 asset.create.upload。",
        ),
        prerequisites=(
