...
开始创建页面时，先查询并复用已有的页面组件；如果当前页面属于有明确模板价值、预计会在多个页面或项目中复用的封面、目录、章节分隔或页面骨架，再创建页面组件。编写页面内容时，如果发现同一职责的指标卡、引用卡、图表或表格模块会跨页重复，且变化可以用稳定的 props/slots 表达，再沉淀为内容组件。只服务当前页面叙事的一次性排版和没有稳定契约的包装容器直接保留在页面源码中，避免为了预先抽象而拆分组件。
...
固定画布中的标题、主体和辅助内容应按区域形成稳定的视觉重心：标题、眉题和导航可以顶部对齐，但主体内容区、内容面板和稀疏卡片默认应在自身可用高度内上下平衡。任何大面积非装饰留白都必须有明确的构图目的；不得仅因为使用 `flex-1`、固定高度或 `mt-auto` 就让内容顶部堆积、尾部被推到底部。页面校验出现 `large_vertical_gap`、`interior_gap`、`trailing_gap` 或 `sparse_top_aligned` 时，默认先修复；只有封面、章节页、海报式构图或明确的固定页脚等场景，才可保留并结合构图判断其合理性。
