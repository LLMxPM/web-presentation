...
def format_ai_session_diagnostics_summary(payload: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- focus_mode: {session['focus_mode']} pinned_project={session['pinned_project_id'] or '-'} focus_version={session['focus_version']}",
        f"- work_scope: {session['work_scope_mode']} allowed_projects={session['allowed_project_ids']}",
...
def _dump_session(session: AiAgentSession) -> dict[str, Any]:
    ...
    return {
        ...
        "allowed_project_ids": list(session.allowed_project_ids_json or []),
        "focus_version": session.focus_version,
