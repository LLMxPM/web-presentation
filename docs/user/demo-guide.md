...
- Demo 地址：[http://49.235.130.241/](http://49.235.130.241/)
- 账号：`Demo`
- 密码：`Demo123456`
...
5. 涉及批量归档、危险覆盖、发布等操作时，先检查工具确认内容，再决定是否执行；内容助手不提供永久删除。
...
Demo 环境系统预置了一个低档位token-plan，额度优先，如果额度用完，会提示模型服务繁忙：
![模型服务繁忙](../assets/模型服务繁忙.png)
如果遇到模型服务繁忙，需要在账号 AI 设置中配置自己的模型和 API Key：


![AI 设置入口](../assets/ai设置入口.png)

1. 打开账户或个人中心中的 AI 设置页面。页面默认进入“内容助手”，左侧可切换“聊天模型”和“图片生成”；内容助手内可切换“模型配置”“工具配置”“系统提示词”和“代码规范”。
2. 聊天模型先选择 Models.dev 目录供应商并录入连接，再选择目录模型或手工填写兼容模型 ID；页面会显示目录最后同步状态，管理员可手动刷新。没有收录的服务可创建自定义 OpenAI-compatible 连接，且必须填写 Base URL。图片生成供应商、API Key、Base URL 和模型配置完全独立，最后回到“内容助手”的“模型配置”完成绑定。
3. 需要调整生成代码约束时，进入“代码规范”，分别切换“页面规范”和“组件规范”，编辑对应 Markdown 后保存；恢复默认会删除该类型的用户覆盖。
4. 保存后回到项目页面或 AI 侧边栏，选择可用配置继续使用。
