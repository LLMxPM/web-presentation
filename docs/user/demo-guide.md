...
1. 打开账户或个人中心中的 AI 设置页面。页面默认进入“内容助手”，左侧可切换“聊天模型”和“图片生成”。
2. 聊天模型先选择 Models.dev 目录供应商并录入连接，再选择目录模型或手工填写兼容模型 ID；页面会显示目录最后同步状态，管理员可手动刷新。没有收录的服务可创建自定义 OpenAI-compatible 连接，且必须填写 Base URL。图片生成供应商、API Key、Base URL 和模型配置完全独立，最后回到“内容助手”的“模型与视觉能力”完成绑定。
