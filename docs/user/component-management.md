...
- 组件类型：当前用于组件库筛选和分类，固定为 `页面组件`、`内容组件`、`原子组件`。
- 组件摘要：说明用途和适用边界。
- 源码内容：当前可编辑草稿。
...
| 类型 | 适用对象 | 尺寸约束 |
| :--- | :--- | :--- |
| 页面组件 | 封面、目录、章节页、页面骨架或整页视觉组件 | 通常依赖页面画布或父级 `h-full w-full` 高度上下文，预览默认不留白 |
| 内容组件 | 卡片、图表、指标组、表格、资源展示块和普通业务区块 | 用于固定布局槽位，必须通过 props 或 `previewSchema` 明确宽高、最小高度、比例、填充方式等尺寸控制 |
| 原子组件 | 页码、角标、图标、主题 Logo、小标签和装饰符号 | 不强制宽高参数，但应提供 `size`、`fontSize`、`padding`、`variant` 等轻量显示尺寸控制 |

数据展示、资源渲染、样式能力和路由能力不再作为组件类型；这些能力通过组件摘要、props、源码依赖和 `previewSchema` 表达。
