...
样式可以应用到项目配置草稿中；应用动作只是填充外层表单，保存后才会写入项目。

## 离线包

样式库支持导出和导入离线包。当前样式离线包 schema_version 为 2，导入时会带上相关样式、主题、资源和字体配置摘要，并先经过预检。
...
4. 在“样式库”页沉淀页面尺寸、基础字号、菜单模式、PDF 按钮和 Markdown 样式规范。
5. 在项目配置中选择主题，或应用样式后保存项目配置。
6. 组件预览需要统一规格时，使用组件预览配置覆盖页面尺寸、主题 key 等预览选项。
