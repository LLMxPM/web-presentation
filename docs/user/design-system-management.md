...
样式库支持导出和导入离线包。当前样式离线包 schema_version 为 2，导入时会带上相关样式、主题、资源、字体配置和建议组件。建议组件会随样式包一起导出源码、预览 schema、组件依赖、组件引用资源和字体配置；导入时会自动创建或复用目标工作空间中的已发布组件，并把导入后的组件重新关联到样式。
