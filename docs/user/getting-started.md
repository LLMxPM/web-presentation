...
页面创建后，可以在编辑区调整代码、展示配置和页面内容，并通过预览区查看结果。Vue 页面还可以切换到“可视化”面板，直接点击画布中的文字、区块或组件，再通过“常用样式”修改内容创作者最常用的字段；定位、网格等能力收在“更多样式”，“页面结构（高级）”默认收起。

可视化面板中的修改会先保留为本地草稿，不会立即覆盖中间画布；点击“保存”后，Backend 才会基于当前页面版本生成新版本并刷新画布。每个已修改字段可以单独恢复原值，也可以撤销全部修改。普通结构、段落和组件可以复制或删除；稳定单层循环中的数据项可以单独复制、删除，循环模板可以整体删除。段落中的链接、组件、动态属性和内联样式会锁定标签外壳，但其静态文本仍可编辑；点击“样式锁定”可以查看被保护的具体标签和属性，“删除锁定样式”会保留并合并标签内文本。动态文本表达式、嵌套循环、复杂 CSS 和未识别 Tailwind 类仍会保持只读，必要时应回到源码编辑器处理。

图片内容应使用 `AssetImage.v1` 组件。选中该组件后，可以从工作空间图片库替换资源、填写替代文本，并设置填充方式和九宫格位置；通用样式只修改外层图片框的宽高、内边距、背景、边框和圆角。原生 `<img>` 不提供低代码图片内容编辑，可改用 `AssetImage.v1`、让 AI 调整，或进入高级源码。
...
在 AI 侧边栏中描述要修改的目标，例如补充内容结构、调整版式、生成组件或替换视觉风格。内容助手可以在当前工作空间跨项目协作；涉及危险覆盖或批量归档时，平台会要求确认后再执行，内容助手不提供永久删除。

更完整的 AI 使用建议见 [AI 协作创作指南](./ai/README.md)。
...
- 想系统管理素材：阅读[资源管理](./workflows/resources.md)。
