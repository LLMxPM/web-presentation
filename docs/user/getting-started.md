...
页面创建后，可以在编辑区调整代码、展示配置和页面内容，并通过预览区查看结果。Vue 页面还可以切换到“可视化”面板，在页面层级或画布中选择容器，以表单方式修改静态文本、组件参数和常用 Tailwind 样式。

可视化面板中的修改会先保留为本地草稿，不会立即覆盖中间画布；点击“保存并刷新”后，Backend 才会基于当前页面版本生成新版本并刷新画布。普通结构、段落和组件可以复制或删除；稳定单层循环中的数据项可以单独复制、删除，循环模板可以整体删除。段落中的链接、组件、动态属性和内联样式会锁定标签外壳，但其静态文本仍可编辑；点击“样式锁定”可以查看被保护的具体标签和属性，“删除锁定样式”会保留并合并标签内文本。动态文本表达式、嵌套循环、复杂 CSS 和未识别 Tailwind 类仍会保持只读，必要时应回到源码编辑器处理。
