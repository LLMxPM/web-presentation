...
编辑这类资源时，应先核对待写入的文本内容、资源类型和用途，再保存。资源库当前不提供内容 diff；写入或替换文件前，平台会生成历史副本。
...
当前资源助手可以管理工作空间资源库，支持资源查询、可编辑文本内容读取/写入、元数据维护、复制和归档。资源助手当前可操作的是 SVG 图标、DrawIO、Mermaid、图表配置、公式等可以文本编辑的资源，不包括图片资源，也暂时无法预览资源的渲染效果。内容助手也可以读取资源列表、可编辑资源内容和资源标签，用于页面生成或改写。
...
资源内容写入前应先核对 AI 准备写入的文本内容；归档和删除类操作应谨慎确认。
