...
1. 登录平台 Web 界面。
2. 进入“账户设置” → “访问令牌 (PAT)”，选择有效期限、授权工作空间与权限 Scope，新建并复制生成的密钥（形如 `wp_pat_xxxxxxxx.yyyyyyyyyyyyyyyy`）。有效期限可选择“长期有效”；工作空间可授权指定空间，或动态授权当前及未来加入的所有工作空间。创建后可在 PAT 管理页编辑名称、有效期限、授权工作空间和 Scope，密钥本身不会变化；已吊销令牌不能编辑。
