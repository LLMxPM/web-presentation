...
公开 Demo 可以使用 [`deploy/scripts/sqlite-demo-backup.sh`](../../../deploy/scripts/sqlite-demo-backup.sh) 保存一份完整数据基线，并使用 [`deploy/scripts/sqlite-demo-restore.sh`](../../../deploy/scripts/sqlite-demo-restore.sh) 定时恢复。两个脚本会从已有容器自动识别实际镜像，以及 `/app/backend/data` 对应的命名 volume 或宿主机目录挂载；操作期间停止 `platform-lite`，完成后重新启动并等待健康检查，无需写死 compose project 生成的 volume 名或宿主机数据目录。
...
可通过环境变量覆盖 `COMPOSE_FILE`、`COMPOSE_PROJECT_NAME`、`COMPOSE_SERVICE`、`DATA_MOUNT_PATH`、`BACKUP_FILE`、`LOCK_DIR` 和 `HEALTH_TIMEOUT_SECONDS`。`DATA_MOUNT_PATH` 表示容器内数据路径，默认是 `/app/backend/data`；即使 Compose 使用 `/home/ubuntu/presentation/data:/app/backend/data` 这样的目录挂载，也不要把它改成宿主机路径。如果部署时使用了 `docker compose -p <name>`，定时任务必须传入相同的 `COMPOSE_PROJECT_NAME`。基线文件包含数据库、用户上传内容以及加密密钥等敏感数据，应限制文件和目录权限。重置会造成短暂不可用，并永久删除基线之后产生的 Demo 数据；不要用于需要保留用户数据的正式环境。升级到包含新数据库迁移的镜像后，应验证兼容性并重新制作基线，避免旧版本镜像与新 schema 混用。
