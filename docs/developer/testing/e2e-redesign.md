...
├── function_model.py        # AI_TEST_MODE=mock 时创建 FunctionModel
...
- fixture setup 先清理同一 E2E namespace 的历史残留；不得只依赖测试函数末尾的清理代码。连续执行 `test:e2e:run` 时也必须可重复。
...
1. **模型层 mock**：保留真实 Chat `provider_key` 与调用协议的一一对应关系，不新增可持久化或可披露的 `provider_key="mock"`。`seed:smoke` 创建使用保留 `e2e-mock-*` model ID 的测试模型配置；仅当 `AI_TEST_MODE=mock` 且 model ID 命中该前缀时，runner/resolver 才将模型替换为 Pydantic AI `FunctionModel`。阶段 2 先支持普通对话和视觉工具 `analyze_visuals` → `generate_image` 链路。
2. **图片生成 mock**：保留 `provider_key="openai_image"`；在 mock 模式且 model ID 命中 `e2e-mock-*` 前缀时注入测试适配器，返回固定、格式合法的小型 PNG 并继续走真实资源落库链路，不发起 HTTP 调用。

`AI_TEST_MODE` 只负责启用测试安全门，不负责选择单个全局场景。`tests/e2e/fixtures/ai-cases.ts` 为每个场景提供唯一、固定的自然语言输入；Backend mock registry 根据规范化后的初始用户输入选择场景，未知输入立即返回明确测试错误。FunctionModel 再根据当前消息历史和工具返回值决定下一步响应。实现不得使用进程级调用计数器或可变的全局 `E2E_AI_SCENARIO`，从而允许多个场景并行执行及 deferred run 自动恢复。
...
- Backend 在 `AI_TEST_MODE=mock` 下通过 `/api/testing/e2e-readiness` 提供不含连接串和密钥的测试就绪信息，至少返回 `test_mode=mock`、`database_profile=e2e`、`redis_profile=e2e` 和 mock scenario 版本；非 mock 模式下该入口返回 404。
...
CI 的 main、定时和 Release 门禁根据测试治理文档选择 `test:e2e` 或 `test:e2e:all`，并同步更新 workflow；不得让 `playwright test` 的默认“运行所有 project”行为改变脚本语义。

### 3.7 失败诊断

- fixture 监听 `requestfailed` 以及 `/api/**` 的 5xx 响应，失败时附加 URL、方法、状态码、响应错误码和截断后的错误摘要；不得保存认证 cookie、API Key 或完整用户内容。
...
| 阶段 | 内容 | 阻塞点 |
| :--- | :--- | :--- |
| 1 | 修复 `ai-visual-tools.smoke.spec.ts` 两个用例；补齐视觉卡片 `data-testid`；stub 从 spec 内联迁到 `fixtures/ai-stub/` 临时公共模块，禁止继续新增 stub 路由 | Editor 小改动，可立即做 |
| 2 | Backend `FunctionModel` 场景执行器、图片测试适配器、真实模型请求熔断和测试环境指纹；补充 mock 安全边界与契约测试 | Backend 改动；不得新增公开 `mock` provider |
| 3 | AI 用例切换为真实 API；storageState 与 auth project；用例目录和 Playwright project 分层；写型用例改用可清理的独立夹具 | 依赖阶段 2 |
| 4 | 默认/回归命令与 CI workflow 收敛；超时调整；失败 API、服务日志和 AI run 自动诊断 | 依赖阶段 3 稳定基线 |
