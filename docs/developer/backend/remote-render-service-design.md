...
- 页面/组件校验区分内容失败与执行不可用；执行不可用不能映射为“源码有错”或“检查通过”。页面合并结果以 `stages.compile` / `stages.render` 表达阶段结论，render=`unavailable` 时顶层 `status=unavailable`，External API `valid` 必须为 false 并返回基础设施错误码。
