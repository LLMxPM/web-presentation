...
用户明确要求把本轮上传图片保存、导入或加入资源库时，统一内容助手调用 `create_entity(resource_type=asset, mode=upload)` 并传入可信 `attachment_id`，由内部 `save_uploaded_image_as_resource` 处理器创建 `asset_type=image` 的工作空间资源。处理器只接受当前用户、当前工作空间、当前会话中 active 且 `source_kind=user_upload` 的附件，不接受 URL、本地路径或 base64，也不开放覆盖已有资源。
