#!/bin/sh
# 文件功能：停止 SQLite 轻量 Demo 服务，并从基线归档完整恢复 lite-data volume。

set -eu
umask 077

# 部署配置：可直接修改默认值，也可通过同名环境变量覆盖。
# 完整 lite-data volume 的基线归档路径。
...
# 在固定 compose 文件上执行命令，避免 cron 工作目录变化影响运行。
compose() {
  docker compose -f "$COMPOSE_FILE" "$@"
}

# 获取已有服务容器，恢复前必须先完成 SQLite 部署。
get_container_id() {
...
[ -s "$BACKUP_FILE" ] || fail "基线不存在或为空：$BACKUP_FILE"
