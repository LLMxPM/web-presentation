...
<template>
  <BaseDialog :model-value="modelValue" :title="style ? '编辑样式' : '新建样式'" width="760px" @update:model-value="handleVisibleChange">
    <div class="space-y-4">
      <section class="space-y-4">
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div class="mt-3 grid grid-cols-3 gap-2 rounded-xl bg-slate-100 p-1">
            <button
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-4">
          ...
          <div class="mt-3 grid grid-cols-2 gap-2 rounded-xl bg-slate-100 p-1">
            <button
    ...
    <template #footer>
      ...
      <BaseButton variant="primary" :loading="loading" @click="handleSave">
        {{ style ? '保存样式' : '创建样式' }}
      </BaseButton>
    </template>
  </BaseDialog>
...
const emit = defineEmits<{
  ...
  save: [payload: WorkspaceStylePayload]
}>()
...
watch(
  () => [props.modelValue, props.style, props.initialStyle] as const,
  ([visible]) => {
    if (visible) {
      syncDraft()
    }
