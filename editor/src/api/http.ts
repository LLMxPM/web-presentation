...
export function getErrorMessage(error: unknown, fallback = '请求失败，请稍后重试。'): string {
  if (axios.isAxiosError(error)) {
    ...
    if (error.response?.status) {
      return buildHttpStatusErrorMessage(error.response.status, fallback)
    }
  }
...
/**
 * 将没有业务错误体的 HTTP 错误转换为中文提示，避免直接暴露 Axios 默认英文状态文案。
 */
function buildHttpStatusErrorMessage(status: number, fallback: string): string {
  ...
  if (status === 409) {
    return fallback === '请求失败，请稍后重试。' ? '当前操作存在冲突，请刷新后重试。' : fallback
  }
  if (status >= 500) {
    return '服务端处理失败，请稍后重试；如反复出现，请联系管理员查看后台日志。'
  }
  return fallback
}
