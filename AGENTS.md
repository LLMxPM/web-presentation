...
内容助手是工作空间级智能体，固定装配少量通用业务工具和特殊工具。`get_operation_guide` 只是从 `tool_specs.py` 返回参数 Schema、约束与示例的普通只读操作手册，不是授权凭证、执行前置条件或额外持久化状态；真实写入始终按当前用户权限、工作空间归属和业务 Schema 校验。内容助手不得注册永久删除能力；单项归档免确认，两个及以上同类型目标必须动态确认并以整批原子语义执行。主题写工具只允许维护 key、name、description 和色板，不得暴露 Logo 或字体配置。
