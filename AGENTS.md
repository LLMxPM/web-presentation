...
当前已落地登录、多用户隔离、工作空间/项目/页面管理、资源库、组件库、主题库、样式库、AI Agent 会话、工具确认、预览、截图、构建和容器发布；完整资源中心、Dashboard、项目关联使用关系和 Runtime 反向回传仍在建设中。
...
- `docs/`：承载平台架构、开发测试、部署、CI/CD、当前状态和测试治理等专题文档。
- `deploy/`：承载 simple、生产可维护、内置依赖 compose 模板和部署环境变量示例。
...
- 工具调用契约和返回示例是系统只读信息；用户只允许编辑智能体描述、业务补充提示词、内容助手 Team 成员描述、工具说明和工具提示词。
...
出现以下情况时，优先更新 `docs/` 下专题文档：

- 本地开发、测试数据、部署、CI/CD、测试策略或项目状态发生变化。
